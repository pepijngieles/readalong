...
function level_filter_summary(array $selected, $allLabel) {
  $allowed = level_codes();
  ...
  for ($i = 1; $i <= $count; $i++) {
    $curr = $ordered[$i] ?? null;
    if ($curr !== null && level_score($curr) === level_score($prev) + 1) {
      $prev = $curr;
      continue;
    }
...
function render_title_menu($id, $pref, $groupLabel, $summaryLabel, $allLabel, array $options, array $selected, array $allowed, $optionLang = false, $multiple = true, $showFlags = false) {
  ...
  ?>
				<div class="home-title-control" data-i18n-all="<?= e($allLabel) ?>"><button type=button class="quiet home-title-trigger" data-click=toggleTitleMenu aria-expanded=false aria-haspopup=listbox aria-controls="<?= e($id) ?>"><span class=visually-hidden><?= e($groupLabel) ?></span><?php if ($showFlags && $selectedCode !== null && lang_flag_path($selectedCode) !== null): ?><span data-title-flag><?= lang_flag($selectedCode, ['decorative' => true]) ?></span><?php endif; ?><span data-title-label<?= $optionLang && $selectedCode !== null ? ' translate=no lang=' . e($selectedCode) : '' ?>><?= e($summaryLabel) ?></span><?php icon('chevron-down', ['size' => 16]); ?></button><div id="<?= e($id) ?>" class=title-menu hidden role=listbox<?= $multiple ? ' aria-multiselectable=true' : '' ?> data-pref="<?= e($pref) ?>"<?= $multiple ? '' : ' data-multiple=false' ?> data-change=updateTitleFilter>
...
function render_browse_filter_menu($id, $pref, $groupLabel, $summaryLabel, $allLabel, array $options, array $selected, array $allowed, $multiple = true) {
  $allSelected = $multiple && language_filter_is_all($selected, $allowed);
  $inputType = $multiple ? 'checkbox' : 'radio';
  ?>
...
<?php $checked = $allSelected || in_array($code, $selected, true); ?>
						<label role=option aria-selected=<?= $checked ? 'true' : 'false' ?>>
							<input type=<?= e($inputType) ?><?= $multiple ? '' : ' name="filter-' . e($id) . '"' ?> value="<?= e($code) ?>"<?= $checked ? ' checked' : '' ?>>
							<?php icon('check', ['size' => 16]); ?>
							<span><?= e($label) ?></span>
						</label>
<?php endforeach; ?>
